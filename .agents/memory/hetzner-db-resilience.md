...
**Tracking-buffer flush retry — keep the two retry paths distinct (do NOT collapse to one count-based cap):** a flush failure is classified by `isTransientFlushError` (pool-checkout OR connection-class). Transient (DB down/restart) → requeued WITHOUT touching `_retryCount`, retained until it ages past `TRACKING_MAX_EVENT_AGE_MS` (default 30min) → survives a multi-minute outage and flushes when DB returns. Non-transient/poison → bounded `MAX_FLUSH_RETRIES` count cap so a bad batch can't loop forever. Memory bounded by `enforceQueueCap()` (drop-oldest at MAX_QUEUE → `queue_full`); aged-out transient drops are `flush_stale`, distinct from poison `flush_error`. **Why:** the old single count-cap (3 retries ≈ 30s) dropped 6,948 events in the 2026-06-05 outage; reverting transient retries to a small count cap reintroduces that loss.
