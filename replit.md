...
- **Subscriber Management:** Features tag-based segmentation with GIN-indexed tags, a refs system for segment targeting, and a cooling-off period for unsubscribed contacts (duration centralized in `server/config/suppression.ts` — `UNSUBSCRIBE_COOLING_OFF_DAYS`, currently 21 days; baked into `subscribers.suppressed_until` at unsubscribe time). Unsubscribes from blocklisted source IPs (`BLOCKED_UNSUBSCRIBE_IPS` in the same file, extendable via `UNSUBSCRIBE_IP_BLOCKLIST` env) auto-tag the subscriber `BCK` + `unsub-ip-<ip>` (permanent segment exclusion; never blocklist mail-provider IPs — RFC 8058 one-click POSTs come from them).
...
- **Segmentation (DSL v2):** Employs a recursive rule DSL for advanced segmentation with nested groups (AND/OR combinators, max 3 depth) and server-side pagination for segments. Includes targeted, cached subscriber counts with auto-invalidation. The optional **`engagement`** field ("engaged_recently"/"not_engaged_recently") filters on the rollup-maintained `subscribers.last_engaged_at` (opens OR clicks; ≤15-min freshness — NOT a real-time event query) over a fixed window (`ENGAGEMENT_RECENCY_DAYS`, 60d, in `server/services/segment-compiler.ts`); all segment query paths funnel through `compileSegmentRules` so count/preview/cursor/audience are auto-covered.
...
- **Bot-Opener DEL Marker:** Daily lease-elected pass (web + worker, `bot_opener_leader`) auto-appends the `DEL` ref to subscribers whose opens are bot-fabricated: over 30 days, opened ≥5 distinct campaigns AND ≥70% of those opens via a `BOT_OPENER_IPS` address (defaults in `server/config/suppression.ts`, env-overridable). Newly marked subscribers get a timestamped `bot_opener_marks` row feeding the `/analytics` "Bot Openers Marked" chart (`GET /api/analytics/bot-openers`); first pass after boot is the retroactive one. Detail: `docs/architecture-history.md`.
