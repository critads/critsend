...
- **CSV Import/Export:** Uses PostgreSQL COPY for high-performance imports with concurrent batch processing (4 parallel COPY operations, 25k rows/batch) and streaming for large exports. Features chunked file uploads up to 1GB. Two-path import pipeline: tag imports (classic merge/override) and refs imports (two-phase with staging → confirmation → merge). Refs imports never touch tags, preserving BCK blacklist safety.
