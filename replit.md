...
- **Robustness:** Includes graceful shutdown, memory monitoring with load shedding, automated campaign auto-resume, bounce webhook idempotency, and bulk-optimized batch webhook processing. Web-process guardians rescue stuck import and campaign jobs. Startup bootstrap migrations use advisory locks and `CREATE INDEX CONCURRENTLY` for safety.
