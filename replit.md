...
- **Real-Time SSE Progress:** Server-Sent Events (`GET /api/jobs/stream`) push instant progress updates for import, flush, and campaign jobs. An in-process `EventEmitter` hub (`server/job-events.ts`) relays events from workers and campaign sender to connected browsers. Frontend `useJobStream()` hook (singleton pattern with ref-counting) updates React Query cache directly, with SSE connection tracking (`isSSEConnected()`) to reduce polling to 30s when SSE is active (fallback to 5s when disconnected). Supports auto-reconnect with exponential backoff. SSE handlers create cache entries when none exist (never drop events due to missing cache). Flush/import mutation `onSuccess` seeds the React Query cache immediately so SSE events are never lost. Terminal SSE events trigger a delayed `invalidateQueries` (500ms) to ensure `completedAt` is fetched from the server.
