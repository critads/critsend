...
- **Process Separation (Phase 5):** `server/dev-launcher.ts` spawns two isolated processes — the web server (`PROCESS_TYPE=web`, pool max=20) and the worker (`PROCESS_TYPE=worker`, pool max=8). `server/worker-main.ts` is the worker entry point. `server/connection-budget.ts` applies PROCESS_TYPE-aware pool sizing, keeping total Neon connections well under the 50-connection limit. The "Start application" workflow runs `tsx server/dev-launcher.ts`.
