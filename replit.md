...
- **Tracking:** Implements open tracking (1x1 pixel) and click tracking (redirects with open-redirect prevention). All tracking URLs are HMAC-SHA256 signed. Captures enriched context (IP, country, device, browser, OS) and uses a link registry for opaque UUID tokens in tracking links, along with branded short tracking URLs. Tracking events are buffered in-memory and flushed in batched multi-row inserts to a dedicated `pg.Pool` for isolation. **Atomic side-effect transactions:** for opens and clicks the `INSERT INTO campaign_stats` and the `UPDATE campaign_sends SET first_open_at/first_click_at` run inside a single trackingPool transaction (BEGIN/COMMIT) — this guarantees that whenever a raw event lands, the headline counter on `campaign_sends` is updated in lock-step. Tag enqueues (`storage.enqueueTagOperation`) remain post-commit and fire-and-forget. **Counter-drift reconciler:** `server/workers/counter-reconciler.ts` runs every 15 min (configurable via `COUNTER_RECONCILE_INTERVAL_MS`) re-deriving `campaigns.sent_count`, `campaign_sends.first_open_at`, and `first_click_at` from the source-of-truth tables for campaigns active in the last `COUNTER_RECONCILE_WINDOW_HOURS=24` (full-table mode via `scripts/reconcile-counters.ts` for one-shot recovery). Metrics: `critsend_counter_drift_fixed_total{counter}`, `critsend_counter_drift_run_duration_ms`, `critsend_counter_drift_last_run_timestamp_seconds` — non-zero `_fixed_total` indicates a regression in the live counter-write path.
