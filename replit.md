...
- **Email Sending:** High-performance campaign sending with configurable speeds (500-60000 emails/min) via Nodemailer with connection pooling. Godzilla mode: 200 concurrent workers, 60k/min rate cap, pooled nullsink transporter (200 maxConnections). Bulk send recording batches DB operations (bulkReserveSendSlots + bulkFinalizeSends) to reduce per-email overhead from 2 DB round-trips to ~2 per batch. Intermediate flush every 500 emails keeps sentCount accurate in near-real-time. Bulk nullsink capture inserts (bulkCreateNullsinkCaptures) replace per-email inserts. Fire-and-forget error logging avoids blocking the send pipeline. Campaign job heartbeat prevents stale cleanup during long sends. SMTP maxConnections scales per-MTA (default 10, configurable via maxSmtpConnections). LISTEN/NOTIFY triggers instant job pickup. Cached status checks (5s TTL) reduce mid-send DB queries. Automatic fallback to individual operations on bulk failure.
- **Sending Engine V2 (Feb 2026):** Two-path architecture: Path A (nullsink) bypasses SMTP entirely via `sendEmailBatchNullsink()` — processes emails synchronously in-memory with pre-computed base HTML template, no TCP/SMTP overhead. Path B (real SMTP) uses chunked concurrency with pooled transporters. Write-behind buffer accumulates results and flushes to DB on 1000-count or 3-second thresholds. Batch size 5000 subscribers per DB fetch. Sub-batching at 1000 for nullsink with status checks between sub-batches.
