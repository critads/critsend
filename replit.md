...
- **Tracking:** Implements open tracking (1x1 pixel) and click tracking (redirects with open-redirect prevention). All tracking URLs are HMAC-SHA256 signed. Captures enriched context (IP, country, device, browser, OS) and uses a link registry for opaque UUID tokens in tracking links, along with branded short tracking URLs. Tracking events are buffered in-memory and flushed in batched multi-row inserts to a dedicated `pg.Pool` for isolation. **Atomic side-effect transactions:** for opens and clicks the `INSERT INTO campaign_stats`, the `UPDATE campaign_sends SET first_open_at/first_click_at`, AND the `UPDATE campaigns SET unique_opens_count/total_opens_count/...` cached counter bump all run inside a single trackingPool transaction (BEGIN/COMMIT). Unsubscribe/complaint events use a parallel `insertBatchAndBumpCounters` txn that pairs the `campaign_stats` insert with `unsubscribes_count`/`complaints_count` bumps. This guarantees that whenever a raw event lands, both the headline counter on `campaign_sends` AND the `/campaigns` list cached counters update in lock-step. Tag enqueues (`storage.enqueueTagOperation`) remain post-commit and fire-and-forget. **Cached engagement counters on `campaigns`:** `unique_opens_count`, `total_opens_count`, `unique_clicks_count`, `total_clicks_count`, `unsubscribes_count`, `complaints_count` — read by `/api/campaigns/stats` in a single fast SELECT instead of aggregating `campaign_stats` on every page load. **Counter-drift reconciler:** `server/workers/counter-reconciler.ts` runs every 15 min (configurable via `COUNTER_RECONCILE_INTERVAL_MS`) re-deriving `campaigns.sent_count`, `campaign_sends.first_open_at`, `first_click_at`, and the six engagement counters from the source-of-truth tables for campaigns active in the last `COUNTER_RECONCILE_WINDOW_HOURS=24` (full-table mode via `scripts/reconcile-counters.ts` for one-shot recovery). Engagement reconciliation is fill-only via `GREATEST()` so it can never lower a real counter on a partial-restore DB. Metrics: `critsend_counter_drift_fixed_total{counter}` (labels: `sent_count`, `first_open_at`, `first_click_at`, `engagement_counters`), `critsend_counter_drift_run_duration_ms`, `critsend_counter_drift_last_run_timestamp_seconds` — non-zero `_fixed_total` indicates a regression in the live counter-write path.
...
- **Process Separation:** Supports a split-process architecture with dedicated web server (main pool 14 + tracking pool 6) and worker (`WORKER_PG_POOL_MAX=18`, `MAX_CONCURRENT_CAMPAIGNS=12`) processes. Total budget 44/50 with headroom 6.
