...
- **Tracking-Pool Isolation:** Open/click/unsubscribe traffic is served by a dedicated `pg.Pool` (`server/tracking-pool.ts`, `PG_TRACKING_POOL_MAX` default 6) that is carved out of the web budget in `server/connection-budget.ts`, so a campaign-blast pixel firehose can never drain the main pool (login/dashboard/imports stay responsive). HTTP handlers in `server/routes/tracking.ts` no longer touch the DB on the request path: they validate the signature, respond with the pixel/redirect/HTML, and call `enqueueTrackingEvent` (server/tracking-buffer.ts) which buffers in-memory and flushes every 1.5 s in batched multi-row INSERTs against the tracking pool. The buffer dedupes `(campaignId, subscriberId, type)` inside a 60 s window (collapses Gmail's "fetch the same pixel 4 times in 2 seconds"), caps queue depth at 50k with drop-oldest + throttled warn, and bulk-batches the recordFirstOpen/recordFirstClick/setSuppressedUntil side effects via `UPDATE … FROM (VALUES …) RETURNING`. A 5k-entry LRU on `getCampaignLinkDestination` keeps warm clicks entirely off the database. The `connect-pg-simple` session store now runs with `createTableIfMissing:false` and the table is created once during startup (no DDL on the request path). Prom metrics: `critsend_tracking_buffer_{enqueued,flushed,dropped,deduped,queue_depth}`, `critsend_tracking_pool_in_use`, `critsend_tracking_link_cache_hits`. Load test: `scripts/load-test-tracking.sh` hammers the open pixel at configurable RPS while polling `/api/health` to assert p99 stays under 1 s.
