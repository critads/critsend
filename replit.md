...
- **Email Sending:** High-performance campaign sending with configurable speeds (500-3000 emails/min) via Nodemailer with connection pooling. Bulk send recording batches DB operations (bulkReserveSendSlots + bulkFinalizeSends) to reduce per-email overhead from 2 DB round-trips to ~2 per batch. Campaign job heartbeat prevents stale cleanup during long sends. SMTP maxConnections scales per-MTA (default 10, configurable via maxSmtpConnections). LISTEN/NOTIFY triggers instant job pickup. Cached status checks (5s TTL) reduce mid-send DB queries. Automatic fallback to individual operations on bulk failure.
